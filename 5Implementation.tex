...
\begin{itemize}
    \item Django Dockerfile was inspired by Anuj Sharma, who created one for his series of Django development guide articles - image used is python:3.9.1-slim, % TODO ref https://dev.to/anujdev/django-development-using-docker-as-host-part-1-dockerfile-3bnc
    \item For Express.js, a latest slim image of \acrshort{lts} node version was chosen - node:14.17.0-slim,
    \item ASP.NET Core has a Dockerfile suggested by the documentation, so it was used in the application - images dotnet:2.1-sdk and dotnet:2.1-aspnetcore-runtime (multistage build). % TODO ref https://docs.microsoft.com/pl-pl/aspnet/core/host-and-deploy/docker/building-net-docker-images?view=aspnetcore-2.1
\end{itemize}
Dockerfiles were also checked by Hadolint, which is a linter that helps to build best practice Docker images. % TODO ref https://github.com/hadolint/hadolint
It verifies if the Dockerfile follows the rules presented in official docker documentation. % TODO ref https://docs.docker.com/develop/develop-images/dockerfile_best-practices/
...
Developers of k6 prepared instructions for usage with Docker % TODO ref https://k6.io/docs/getting-started/running-k6/#running-local-tests
as well as an example Docker Compose configuration, including InfluxDB and Grafana integration. % TODO ref https://github.com/k6io/k6/blob/master/docker-compose.yml
...
\section{ASP.NET}
\subsection{Model}
% TODO say it handles serialization
